...
if doping_str == "0.1\\%":
    ...
    raise ValueError(f"Invalid string for doping concentration: {doping_str}. Choose only \"0.1\\%\".")
...
captures_str = f"Number of lithium-6 captures: {li6_captures}\nNumber of hydrogen captures: {total_captures - li6_captures}\nTotal number of captures: {total_captures}"
print(captures_str)
neutron_data = filtered_data[filtered_data["trackPDG"] == pdg_codes[2]]
# Get the time of last neutron capture point in microseconds
neutron_time_data = neutron_data.groupby("Row").last().reset_index()
neutron_time = 0.001 * neutron_time_data["trackTime"]
# Mean and Standard Deviation
mean_val = np.nanmean(neutron_time) # mean time of capture
cap_mean_str = "Mean capture time on " + doping_str + f" $^6$Li: {mean_val:.0f} $\\mu$s"
print(cap_mean_str)
std_dev = np.nanstd(neutron_time) # std dev
cap_std_dev_str = f"Standard deviation: {std_dev:.0f} $\\mu$s"
print(cap_std_dev_str)
# Compute histogram bins
bin_edges = np.linspace(0, 100, 101)
hist_values, _ = np.histogram(neutron_time, bins=bin_edges)
# Plot using stairs
plt.stairs(hist_values, bin_edges, color="black", linestyle=plot_styles[doping_str], label=f"{doping_str}", alpha=1.0)
print("Done!")
plt.xlim(0,100)
plt.xlabel("Capture time of IBD Neutrons ($\\mu$s)")
plt.ylabel("Counts")
plt.legend(title="\\% wt $^6$Li doping", framealpha=1.0)
plt.grid()
# Adjust layout
plt.tight_layout()
plt.savefig("plots/neutron_capture_time_" + run_nums[rnum] + ".pdf", bbox_inches='tight')
plt.show()
