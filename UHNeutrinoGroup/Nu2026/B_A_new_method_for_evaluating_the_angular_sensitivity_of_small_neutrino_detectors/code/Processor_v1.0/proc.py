...
class DataProcessor():
    ...
    def plotHistogram(self, type_="positron", save=False):
        ...
        # Plots histogram.
        if type_ == "neutron":
            plt.hist(track_lengths, bins=2048, color="orange")
            plt.xlim(0, 300)
...
if __name__ == "__main__":
    ...
    #proc.angularDist()
    #proc.angularDist(type_="first-scatter")

    #proc.plotTracks(10)
    """
    Takes tuple (elev, azim, roll).
    """
    #proc.plotTracks(1000, view=[20,45,0], save=False)
    proc.plotTracks(1000, save=False)

    #proc.printBinDists()

    #proc.plotHistogram()
    #proc.plotHistogram(type_="neutron")
    sys.exit()
